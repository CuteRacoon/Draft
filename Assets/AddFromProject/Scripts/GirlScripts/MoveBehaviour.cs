...
public class MoveBehaviour : GenericBehaviour
{
    ...
    {
        if (behaviourManager.InCutscene) // добавь это условие
        {
            behaviourManager.GetRigidBody.useGravity = true;

            Vector2 dir = new Vector2(behaviourManager.CutSceneH, behaviourManager.CutSceneV);
            speed = Vector2.ClampMagnitude(dir, 1f).magnitude * walkSpeed*1.2f; // Принудительно скорость ходьбы
            behaviourManager.GetAnim.SetFloat(speedFloat, speed, speedDampTime, Time.deltaTime);

            // Вращаем персонажа в нужном направлении
            Rotating(behaviourManager.CutSceneH, behaviourManager.CutSceneV);
            return;
        }
        else
        {
